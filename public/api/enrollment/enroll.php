<?php
// Return JSON in all cases, avoid accidental HTML output
if (session_status() === PHP_SESSION_NONE) session_start();
header('Content-Type: application/json');
try {
    // suppress accidental output from included files
    ob_start();
    ...
    // helper: check users table for id
    if ($sess_user) {
    ...
    // If still not resolved, try resolving student session (which may be students.sid) -> map by email to users
    if (!$user_id && $sess_student) {
        // First check if users row with that id exists (legacy where student_id was user id)
        $chk2 = $conn->prepare("SELECT id FROM users WHERE id = ? LIMIT 1");
        if ($chk2) {
            $chk2->bind_param('i', $sess_student);
            $chk2->execute();
            $chk2->store_result();
            if ($chk2->num_rows > 0) {
                $user_id = $sess_student;
            }
            $chk2->close();
        }
    }
    ...
    if (!$user_id && $sess_student) {
        // try map students.sid -> users.id by email
        $s = $conn->prepare("SELECT email FROM students WHERE sid = ? LIMIT 1");
        if ($s) {
            $s->bind_param('i', $sess_student);
            $s->execute();
            $res = $s->get_result();
    ...
    // Validate resolved user exists
    if ($user_id <= 0) {
        http_response_code(400);
        error_log('[enroll_course] invalid resolved user_id from session: ' . json_encode([ 'sess_user'=>$sess_user, 'sess_student'=>$sess_student ]));
    ...
    if ($stmt->execute()) {
        ...
        echo json_encode(['success' => false, 'message' => 'Failed to enroll: ' . $conn->error]);
    }
    ...
    http_response_code(500);
    error_log('[enroll_course] ' . $ex->getMessage());
