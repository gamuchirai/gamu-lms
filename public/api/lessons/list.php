...
// public/api/lessons this is three levels up.
...
// If instructor, ensure they are assigned to the course (if mapping exists)
...
if ($role === 'instructor' && $check && $check->num_rows > 0) {
    ...
    $vr->bind_param('ii', $course_id, $user_id);
    $vr->execute(); $vr->store_result();
...
$stmt->execute();
$res = $stmt->get_result();
$lessons = $res ? $res->fetch_all(MYSQLI_ASSOC) : [];
$stmt->close();
$conn->close();
